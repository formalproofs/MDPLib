...
% For shared-counter theorems (\newtheorem{x}[theorem]{X}) cleveref needs each
% environment to have its own counter aliased to the shared one, otherwise every
% \cref comes out as "Theorem".  The June 2026 LaTeX kernel does that aliasing
% itself (\newcounteralias); before that it took thmtools, whose aliasctr does the
% same thing.  Loading both aliases the counter twice and aborts the run with
% "Command \c@proposition already defined", so load thmtools only on kernels that
% lack the built-in.
\ifdefined\newcounteralias\else
  \usepackage{thmtools}
\fi
